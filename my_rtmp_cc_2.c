...
struct bpf_bictcp {
    ...
    /* 新規追加: 輻輳検出時のタイムスタンプと待機フラグ */
    __u32 cong_detect_time; /* Timestamp when congestion was detected */
    __u8 cong_waiting;      /* Flag indicating waiting period */
};
...
static void bictcp_reset(struct bpf_bictcp *ca)
{
    ...
    ca->tcp_cwnd = 0;
    ca->found = 0;
    /* 新規追加: 輻輳検出関連のフィールドをリセット */
    ca->cong_detect_time = 0;
    ca->cong_waiting = 0;
}
...
SEC("struct_ops")
void BPF_PROG(bpf_rtmp_cc_2_init, struct sock *sk)
{
    struct bpf_bictcp *ca = inet_csk_ca(sk);
...
SEC("struct_ops")
void BPF_PROG(bpf_rtmp_cc_2_cwnd_event, struct sock *sk, enum tcp_ca_event event)
{
...
SEC("struct_ops")
void BPF_PROG(bpf_rtmp_cc_2_cong_avoid, struct sock *sk, __u32 ack, __u32 acked)
{
    struct tcp_sock *tp = tcp_sk(sk);
    struct bpf_bictcp *ca = inet_csk_ca(sk);
...
SEC("struct_ops")
__u32 BPF_PROG(bpf_rtmp_cc_2_recalc_ssthresh, struct sock *sk)
{
...
SEC("struct_ops")
void BPF_PROG(bpf_rtmp_cc_2_state, struct sock *sk, __u8 new_state)
{
    if (new_state == TCP_CA_Loss) {
        struct bpf_bictcp *ca = inet_csk_ca(sk);

        /* 新規追加: 輻輳検出時に待機フラグとタイムスタンプを設定 */
        if (!ca->cong_waiting) {
            ca->cong_detect_time = tcp_jiffies32;
            ca->cong_waiting = 1;
        }
...
SEC("struct_ops")
void BPF_PROG(bpf_rtmp_cc_2_acked, struct sock *sk, const struct ack_sample *sample)
{
    const struct tcp_sock *tp = tcp_sk(sk);
    struct bpf_bictcp *ca = inet_csk_ca(sk);
    __u32 delay;
...
SEC("struct_ops")
__u32 BPF_PROG(bpf_rtmp_cc_2_undo_cwnd, struct sock *sk)
{
    return tcp_reno_undo_cwnd(sk);
}

SEC(".struct_ops")
struct tcp_congestion_ops my_rtmp_cc_2 = {
    .init = (void *)bpf_rtmp_cc_2_init,
    .ssthresh = (void *)bpf_rtmp_cc_2_recalc_ssthresh,
    .cong_avoid = (void *)bpf_rtmp_cc_2_cong_avoid,
    .set_state = (void *)bpf_rtmp_cc_2_state,
    .undo_cwnd = (void *)bpf_rtmp_cc_2_undo_cwnd,
    .cwnd_event = (void *)bpf_rtmp_cc_2_cwnd_event,
    .pkts_acked     = (void *)bpf_rtmp_cc_2_acked,
    .name = "my_rtmp_cc_2",
};
